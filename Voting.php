...
$sql = "SELECT * FROM  $table WHERE divi='$div'and gender='male' ";
    ...
    while($row = $result->fetch_assoc())
    {
      echo"				<div class='col-sm-4'>
							<div class='cards' id='c1'>
								...
								<div class='row'>								
									<div class='col-sm-12'>
									Candicate Name : <b>".$row['name']."</b><br/>
									</div>
								</div>
								...
								<div class='row'>
									<div class='col-sm-12'>
										<form action='VoteControl.php' method='post'>
											...
											<input type='hidden' name='nameOfUser' value='".$row['name']."'>
											<input type='hidden' name='emailOfUser' value='".$row['email']."'>
...
{
    echo "<center>No Male Candidates</center>";
}
echo "</div></div>";
$sql2 = "SELECT * FROM  $table";
$result2 = $conn->query($sql2);
...
$sql = "SELECT * FROM  $table WHERE divi='$div'and gender='female' ";
$result = $conn->query($sql);
if ($result->num_rows > 0)
{
    while($row = $result->fetch_assoc())
...
{
    echo "<center>No Female Candidates</center>";
}
echo "</div></div>";
$conn->close();
?>
